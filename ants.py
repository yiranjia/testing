    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                if self.ant.container == True and self.ant.ant == None and insect.container == False:
                    self.ant.contain_ant(insect)

                elif insect.container == True and insect.ant == None and self.ant.container == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 3B
        def find_bees(place):
            bees_at_places, is_end = [], False
            while not is_end:
                bees_at_places.append(place.bees)
                place = place.entrance
                if place == None or place == hive:
                    is_end = True
            return bees_at_places

        bees_in_range,targets = find_bees(self.place)[self.min_range:self.max_range], []

        index = 0
        while index<len(bees_in_range):
            bees = bees_in_range[index]
            if bees:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        if self.noqueen:
            ...
            QueenAnt.noqueen=False
            self.real=True
            self.list_of_doubled=[]
        else:
            ScubaThrower.__init__(self)
            self.real=False
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if not self.real:
            ...
            while exit != None:
                if exit.ant and exit.ant not in self.list_of_doubled and exit.ant.container==False:
                    exit.ant.damage *= 2
                    self.list_of_doubled.append(exit.ant)

                elif exit.ant and isinstance(exit.ant,BodyguardAnt):
                    if exit.ant.ant and exit.ant.ant not in self.list_of_doubled:
                        exit.ant.ant.damage *= 2
                        self.list_of_doubled.append(exit.ant.ant)

                    if isinstance(exit.ant,TankAnt) and exit.ant not in self.list_of_doubled:
                        exit.ant.damage *= 2
                        self.list_of_doubled.append(exit.ant)
